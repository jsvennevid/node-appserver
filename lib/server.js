...
_.extend(Server.prototype, {
            ...
            function (callback) {
                ...
                io.on('connection', function (socket) {
                    if (config.web.proxy.allow && socket.handshake.headers.hasOwnProperty(config.web.proxy.address)) {
                        ...
                        socket.handshake.address.address = address.split(/*, */)[0];
                    }
                });
    ...
    createMiddleware: function () {
        ...
        var staticMatch = /\/static\/[^\/]+(\/.*)/;
        app.use(function (req, res, next) {
            var m = staticMatch.exec(req.url);
            if (m) req.url = m[1];
            next();
        });
    },
    ...
    createBaseRequests: function () {
        ...
        app.get('/socket.io/socket.io.js', this.__io.serve.bind(this.__io));
    },
