...
_.extend(Application, {
    ...
    create: function (options) {
        ...
        if (config.preload) {
            _.each(app.files, function (file, name) {
                console.log("Pre-loading '" + name + "'");

                file.generate(function (err, data) {
                    if (err) {
                        console.log("Error pre-loading '" + name + "'");
                        return;
                    }
                    ...
                    if (data) {
                        ...
                        file.generate(function (err, data) {
                            if (!err) {
                                ...
                                res.sendResponse("File not found", 404);
                            }
                        });
                    }
...
_.extend(Application.prototype, {
    getMatches: function (specs) {
        ...
        _.each(specs, function (spec) {
            ...
            if (filespec[0] == '/') {
                var match = [filespec];
                if (root) {
                    match = glob.sync(path.join(root, filespec));

                    match = _.map(match, function (file) {
                        var ofs = file.indexOf(root);
                        if (ofs < 0) {
                            return file;
                        }
                        return file.slice(ofs + root.length);
                    });
                }
                files = files.concat({files: match, tags: tags});
                return;
            }
    ...
    generateBundle: function (bundle, callback) {
        var errors = 0;
        var result = '';
        var process = function (index) {
            var entry = bundle[index];
            entry.generate(function (err, data) {
                ...
                if (index == (bundle.length - 1)) {
                    callback(errors > 0 ? "Failed to build bundle" : null, result);
                } else {
                    process(index + 1);
                }
            });
    ...
    buildScripts: function (name, matches) {
        ...
        _.each(matches, function (match) {
            _.each(match.files, function (file) {
                ...
                if (!match.tags && config.production) {
                    bundle.push({
                        target: target,
                        bname: bname,
                        generate: generate
        ...
        if (bundle.length > 0) {
            this.files['scripts/' + name + '/' + name + '.js'] = {
                generate: _.bind(function (callback) {
                    this.generateBundle(bundle, function (err, data) {
                        if (err) {
                            callback(err);
                            return;
                        }
                        ...
                        if (config.compress) {
                            if (_.isUndefined(UglifyJS)) {
                                UglifyJS = require('uglify-js');
                            }

                            var compressed = UglifyJS.minify(data, {
                                fromString: true
                            });
                            callback(null, compressed.code);
                        } else {
                            callback(null, data);
                        }
                    });
            ...
            this.files['styles/' + name + '/' + name + '.css'] = {
                generate: _.bind(function (callback) {
                    this.generateBundle(bundle, function (err, data) {
                        ...
                        if (config.compress) {
                            if (_.isUndefined(sqwish)) {
                                sqwish = require('sqwish');
                            }

                            var compressed = sqwish.minify(data);
    ...
    buildApplication: function (uses, inherits) {
        ...
        var generateAppCache = _.bind(function (callback) {
            ...
            if (config.production) {
                ...
                if (this.cache) {
                    result += "# Application\n";
                    _.each(this.getMatches(this.cache), function (entry) {
        ...
        this.files[this.name + '.appcache'] = { generate: generateAppCache, type: 'text/cache-manifest'};
    }
});
