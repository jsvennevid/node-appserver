...
function Application()
{
    ...
    this.urlFilter = /\/([^?]*)(?:\?.*)?/;
    this.timestamp = new Number(Math.floor(new Date().getTime() / 1000)).toFixed(0); // TODO: allow for pre-packaged builds
}

var _globalTimestamp = new Number(Math.floor(new Date().getTime() / 1000)).toFixed(0);
var _globalHash = crypto.createHash('sha1').update('appserver').update(_globalTimestamp).digest('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
...
_.extend(Application.prototype, {
    ...
    express: function () {
        return function (req, res, next) {
            var file;
            do {
                var url = this.urlFilter.exec(req.url);
                if (url) {
    ...
    build: function () {
        ...
        shasum.update(this.options.name);
        shasum.update(this.timestamp);
        this.hash = shasum.digest('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
        ...
        _.each(this.options.templates, function (templates, name) {
            ...
            this.compile(matches, name, this.templates, 'templates', 'text/javascript', function (file, root) {
                ...
                var bname = rname + '.js';
                return path.normalize(path.join(hash, root, file.group, bname)).split(path.sep).join('/');
...
/*                    var output = new CleanCSS().minify(input);
            ...
            async.eachSeries(_.keys(groups), function (name, callback) {
                ...
                async.eachSeries(groups[name], function (bundle, callback) {
                    ...
                    async.concatSeries(files, function (entry, callback) {
                        ...
                        var filename = path.normalize(path.join(this.hash, type, name, name + (current++) + path.extname(_.first(files).name))).split(path.sep).join("/");
    ...
    gather: function (target, key, use) {
        ...
        _.each(types[use], function (entry) {
            ...
            if (file[0] != '/') {
                ...
                target.push({
                    name: this.getCdnUrl(absolute, true),
                    tags: entry.tags,
                    meta: entry.meta
                });
            } else {
                target.push({
                    name: this.getCdnUrl(entry.name),
                    tags: entry.tags,
                    meta: entry.meta
                });
            }
    ...
    publish: function publish(uses, callback) {
        ...
        data.scripts.unshift({
            tags: undefined,
            name: this.getCdnUrl(this.getPrimaryTarget() + this.hash + '/config.js', true)
        });
        ...
        var generateConfig = function (callback) {
            var config = {
                hash: this.hash,
                global_hash: Application.getGlobalHash(),
        ...
        var dateTime = new Date(this.timestamp * 1000).toString();
        ...
        this.files[''] = index;
        this.files[this.hash + '/config.js'] = configFile;
    ...
    getCdnUrl: function (url, local) {
        var cdn_prefix = this.getContentDistribution();
        if (!!local) {
            return cdn_prefix + url;
        } else {
            var global_hash = Application.getGlobalHash();
            return (!!cdn_prefix ? cdn_prefix + '/static/' + global_hash : '') + url;
        }
    }
});
